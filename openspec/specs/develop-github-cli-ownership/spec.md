...
## Purpose
Define how develop hosts provide GitHub CLI as user-scoped tooling without a repo-managed WSL compatibility fallback path.
...
### Requirement: WSL compatibility wrappers do not declare a gh-specific fallback
WSL develop-host configuration SHALL not add a repo-managed compatibility entry for `/usr/bin/gh`.

#### Scenario: compatibility fallback omits gh
- **WHEN** the WSL compatibility wrapper configuration is inspected after this change
- **THEN** it SHALL not create a `gh` entry under the fallback output

#### Scenario: General envfs support remains disabled
...
#### Scenario: Workflow docs stop promising a repo-managed gh compatibility path
- **WHEN** active workflow documentation is inspected after this change
- **THEN** it SHALL not state that the repo provides `/usr/bin/gh` through a WSL compatibility wrapper
