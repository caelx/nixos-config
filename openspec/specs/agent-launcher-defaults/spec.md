...
The repo SHALL document that the develop-host `codex`, `gemini`, and `opencode` launchers default to YOLO or allow-all execution, that automatic maintenance runs through a systemd timer instead of launch-time wrapper hooks, that managed OpenSpec and external Caveman installs are refreshed through that maintenance path, that stale removed-tool Codex hook state is cleaned during managed develop-host convergence, and that the change takes effect after the relevant rebuild or switch.
...
- **WHEN** active workflow documentation is inspected
- **THEN** it SHALL state that agent CLI updates, managed `skills.sh` installs, shared skill refresh, Gemini extension refresh, `agent-browser` bootstrap, and OpenCode model refresh happen through the scheduled maintenance service rather than on each launcher start
...
- **THEN** it SHALL state that managed develop-host convergence cleans stale Codex hook entries for removed repo-managed tooling and that already-running Codex sessions may need a restart to observe the cleaned state
