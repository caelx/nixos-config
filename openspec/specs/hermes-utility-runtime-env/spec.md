# hermes-utility-runtime-env Specification

## Purpose
Define the container-wide Hermes env contract that Ghostship manages on
`chill-penguin` while upstream `ghostship-hermes` owns rendering the managed
profile `.env` files.
## Requirements
