# changedetection-service Specification

## Purpose
TBD - created by archiving change add-changedetectionio-stack. Update Purpose after archive.

## Requirements
### Requirement: Ghostship SHALL manage changedetection as a self-hosted service

The server-host stack SHALL include a declarative `changedetection` service
...
- **THEN** a repo-managed `changedetection` container definition is emitted
- **AND** the service stores durable state under `/srv/apps/changedetection`
...
- **AND** that entry points at the managed `changedetection` container
