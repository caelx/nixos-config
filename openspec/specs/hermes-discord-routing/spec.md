...
`GHOSTSHIP_CODEX_CHANNEL` as the repo-owned forced Codex channel.
...
- **AND** the free-response channel list SHALL include
...
- **AND** the supported contract SHALL not require `GHOSTSHIP_ROUTER_CHANNEL`
...
- **AND** the deployed contract SHALL not depend on `GHOSTSHIP_ROUTER_CHANNEL`

## ADDED Requirements

### Requirement: Retired router Discord lane SHALL not remain a supported contract path
The self-hosted Hermes runtime on `chill-penguin` SHALL not treat the retired
router-pinned Discord lane as part of the supported downstream routing
contract.
...
- **THEN** the session SHALL follow the normal runtime routing path for that
  session
- **AND** the session SHALL not require a repo-owned forced router route

#### Scenario: Stale retired env does not restore a supported pinned lane
- **WHEN** operators still have a stale `GHOSTSHIP_ROUTER_CHANNEL` value from an
  older deployment
...
- **AND** the refreshed runtime contract SHALL describe only
  `GHOSTSHIP_CODEX_CHANNEL` as a repo-owned forced Discord lane
