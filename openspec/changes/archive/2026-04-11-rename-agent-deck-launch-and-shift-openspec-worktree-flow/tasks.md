...
- [x] 1.1 Update `home/profiles/develop.nix` so the managed launcher command,
...
- [x] 1.2 Update active references to the launcher in the repo docs and
...
- [x] 2.1 Update `modules/develop/agent-tooling.nix` so the generated
  ...
  do.
- [x] 2.2 Update `modules/develop/agent-tooling.nix` so the generated
...
- [x] 2.3 Update `modules/develop/agent-tooling.nix` so the generated
...
- [x] 2.4 Refresh the checked-in OpenSpec skill and command surfaces that carry
...
- [x] 3.1 Update the active OpenSpec specs under
  ...
  behavior.
- [x] 3.2 Run `nix flake check --no-build -L` from the repo root and run a
  ...
  cleanly.
- [x] 3.3 Run `openspec status --change rename-agent-deck-launch-and-shift-openspec-worktree-flow`
