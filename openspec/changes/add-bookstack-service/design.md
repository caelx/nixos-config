...
- Surface BookStack in Homepage under the existing `Services` group and place it in Muximux immediately after Prowlarr.
...
### Treat Homepage placement as part of the BookStack capability and Muximux ordering as a spec delta

Homepage owns the `Services` grouping declaratively in `modules/self-hosted/homepage.nix`, so BookStack visibility there remains part of the BookStack capability. Muximux already has an existing placement capability, so the requested BookStack insertion after Prowlarr and the Chaptarr dropdown move before Bazarr should be captured as a delta to `muximux-service-placement` instead of as a second new capability.

Alternatives considered:
- Model Homepage and Muximux placement as one new dashboard capability. Rejected because Muximux already has an established placement spec surface.
- Keep Muximux out of scope. Rejected because the user explicitly requested the BookStack and Chaptarr Muximux changes.
...
3. Add the Homepage `Services` entry for BookStack and update Muximux so BookStack appears after Prowlarr while Chaptarr moves into the dropdown before Bazarr.
...
6. Apply the host configuration on `chill-penguin` and verify the containers, persistent state, Homepage entry, Muximux placement, Hermes runtime env projection, and any required ingress follow-up.
