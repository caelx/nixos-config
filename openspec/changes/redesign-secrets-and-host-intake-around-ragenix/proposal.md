...
- Manual implications: all hosts will need a rebuild to pick up the new secret runtime wiring; the repo will need a one-time migration from `secrets.yaml` into logical-unit `.age` files; operators will stop using the old `sops-*` commands; each operator will need to create and register a dedicated passwordless edit key that is not their default SSH key; WSL2 hosts will need bootstrap-time host-key generation before intake capture; temporary host-intake directories under `references/host-intake/` must be removed after Codex finishes integration.
