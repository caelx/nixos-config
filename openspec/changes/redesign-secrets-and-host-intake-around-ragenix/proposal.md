...
- Replace `sops-nix`, `.sops.yaml`, and the repo-wide encrypted `secrets.yaml` workflow with `ragenix`, `secrets/recipients.nix`, `secrets/catalog.nix`, a retained ignored plaintext mirror, and logical-unit `.age` files.
- Standardize machine secret recipients on SSH host `ed25519` public keys and require a dedicated passwordless operator edit key that is separate from the user's default SSH key, with reusable recipient groups composed in `recipients.nix`.
...
- Redesign bootstrap so host capture produces a temporary intake artifact bundle containing host metadata, the standalone `hardware-configuration.nix`, and the host SSH `ed25519` public key, while explicitly ensuring WSL2 hosts generate that host key before capture.
...
- Remove legacy `sops-*` helper tooling and stale docs that reference the pre-redesign commands while keeping the plaintext mirror as the normal edit surface.
...
- Manual implications: all hosts will need a rebuild to pick up the new secret runtime wiring; the repo will need a one-time migration from `secrets.yaml` into logical-unit `.age` files while retaining `secrets.dec.yaml` as the ignored plaintext mirror; operators will stop using the old `sops-*` commands; each operator will need to create and register a dedicated passwordless edit key that is not their default SSH key; WSL2 hosts will need bootstrap-time host-key generation before intake capture; temporary host-intake directories under `references/host-intake/` must be removed after Codex finishes integration.
