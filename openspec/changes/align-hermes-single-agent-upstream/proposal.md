...
- **BREAKING** Remove the repo-owned Hermes profile fleet (`assistant`, `operations`, `supervisor`) from the supported `chill-penguin` runtime contract and replace it with one managed agent rooted at `/home/hermes/.hermes`. During apply we confirmed the current published `ghcr.io/caelx/ghostship-hermes:latest` image still exposes `hermes-agent.service` rather than the newer `ghostship-hermes-*` unit set from upstream source, so host startup now supports that published-image fallback while keeping the rest of the single-agent contract aligned to upstream.
