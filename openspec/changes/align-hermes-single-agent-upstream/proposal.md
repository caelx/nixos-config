...
- **BREAKING** Remove the repo-owned Hermes profile fleet (`assistant`, `operations`, `supervisor`) from the supported `chill-penguin` runtime contract and replace it with one managed agent rooted at `/home/hermes/.hermes`, using the image-owned `ghostship-hermes-startup.service` and `ghostship-hermes-user-tooling-refresh.timer` path from current upstream images.
