...
- **Homepage group rebalance**: Rebalanced only Homepage's `Services`, `Management`, `Utilities`, and `Infrastructure` columns while leaving the existing `Media`, `Automation`, and `Downloads` groups intact. `Services` now holds the day-to-day apps, `Management` holds dashboards plus control-plane tools like `n8n`, Changedetection, and the CloakBrowser manager, `Utilities` holds helper/search/conversion tools, and `Infrastructure` now focuses on backing caches, sidecars, and service databases including the Firecrawl Playwright/Postgres/RabbitMQ/Redis stack.
