...
### Changed
- **OpenSpec agent workflow**: Replaced the old Superpowers-based CLI workflow with a pinned OpenSpec flake input, added the `openspec` CLI to the shared develop toolchain, checked in repo-local OpenSpec scaffolding for Codex/Gemini/OpenCode, and migrated active planning references from `docs/superpowers` to the repo-root `openspec/` tree.
