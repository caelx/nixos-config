...
- **Boomer Xemu first-run setup**: Launch Xemu with the Nix-generated
  ...
  and HDD files are present. Hide Xemu's menubar and convert large redump-style
  Xbox ISOs to cached XISO images before launch so games do not boot to the
  "please insert an Xbox disc" video partition.
...
- **Boomer Switch Pro controller alignment**: Align generated controller maps
  ...
  right stick looks up/down and left/right, Select/Minus toggles map,
  Y toggles crouch, X is unbound, L/R select previous/next weapon, ZL is alt
  fire, ZR is fire, `+` opens the menu, and Square/Capture is unbound. Seed GZDoom's SDL
