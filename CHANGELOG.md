...
- **Homepage network stats mount**: Homepage now mounts host `/sys` read-only and pins the resources widget to `end0` on `chill-penguin`, avoiding the broken `/sys/class/net` symlinks and container-default-interface mismatch that left network activity blank.
