...
## Gemini CLI Expert Skills

Expert skills should be managed using the **Imperative Bootstrap Pattern** within the Gemini NixOS module (`modules/common/gemini.nix` or equivalent). This ensures that skills are automatically installed and kept in sync with the repository without needing manual user intervention.

### Guidelines for Skills Deployment
- **Storage**: Skill source files (Markdown, scripts, etc.) should reside in `home/config/skills/<skill-name>/`.
- **Packaging**: Skills must be packaged into a `.skill` file (ZIP format) within their source directory for deployment.
- **Bootstrap Logic**:
    - The `base-gemini` script in the Nix module must check for the skill's presence in `~/.gemini/skills/`.
    - If missing, it should use `gemini skills install <nix-store-path-to-skill-file>` to perform the installation.
    - Example:
      ```bash
      if [ ! -d "$HOME/.gemini/skills/nix" ]; then
        npx -y @google/gemini-cli skills install ${./path/to/nix.skill} --scope user --consent
      fi
      ```
- **Activation**: New skills should be added to the `skills.default` array in the `settings.json` generated by the module to ensure they are active by default.
- **Validation**: Always perform a dry-run build (`nh os build`) after adding or updating a skill's deployment logic to ensure correct path resolution and syntax.

### Rationale
This pattern provides a robust middle ground between fully declarative file management (which can be fragile for complex directory structures with scripts) and manual installation (which is error-prone and doesn't scale across hosts).
