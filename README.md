...
Hermes now follows the current upstream `ghostship-hermes` `main` workstation contract on `chill-penguin`: `/srv/apps/hermes/home` mounts at `/home/hermes`, `/srv/apps/hermes/workspace` mounts at `/workspace`, and `/srv/apps/hermes/nix` mounts directly at `/nix`. The image owns its internal supervision, first-boot `/nix` seeding, and fixed runtime env, so `podman-hermes` no longer tries to start in-container `systemd` units or pre-populate `/nix` from the host. Ghostship passes only the supported downstream-facing env surface, including `DISCORD_ALLOWED_USERS`, `DISCORD_FREE_RESPONSE_CHANNELS`, `GHOSTSHIP_ROUTER_CHANNEL`, and `GHOSTSHIP_CODEX_CHANNEL`; the free-response channel list is rendered to include the router lane `1492841053642817606`, the Codex lane `1493462179725180959`, and the current three Ghostship free-response channels `1491229269127598281`, `1491229248856260799`, and `1491229299452412044`. The repo-managed `skill-creator` and Crush Crawfish `SOUL.md` defaults are copied directly into `/srv/apps/hermes/home/.hermes/skills/skill-creator/` and `/srv/apps/hermes/home/.hermes/SOUL.md` only when those runtime-owned paths are missing so later operator edits stay intact. This image cutover is intentionally destructive: stop the Hermes container, remove `/srv/apps/hermes/home`, `/srv/apps/hermes/workspace`, and `/srv/apps/hermes/nix`, then start the updated image against clean persisted directories and let it reseed `/nix` and the runtime from scratch. Muximux keeps PriceBuddy in the dropdown immediately after Bazarr.
