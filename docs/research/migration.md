...
3.  **Unpack Metadata**: 
    ...
    ```
4.  **Switch Config**: From a root shell, run `nixos-rebuild build --flake .#chill-penguin` and then `./result/bin/switch-to-configuration switch` to finalize the system state.
