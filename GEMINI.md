...
- **Ghostship YAML Paths**: `ghostship-config.py` YAML paths must handle dots inside bracket selectors (for example `[Infrastructure].[Llama.cpp]`) and bracketed numeric list indices (for example `custom_formats[0].trash_ids[0]`). Homepage and Recyclarr both depend on those exact path forms.
