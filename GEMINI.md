...
- **Cloudflared Widget IDs**: Homepage's Cloudflared widget should receive raw `accountid` and `tunnelid` strings. Wrapping those values in quotes causes Homepage to request malformed Cloudflare API paths and the widget crashes with a null `connections` object.
