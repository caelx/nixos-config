...
- **Cloudflared Widget Auth**: Homepage's Cloudflared widget needs the Cloudflare API token for the account, not the tunnel token used by the `cloudflared` container. Keep `accountid` and `tunnelid` raw, and source `CLOUDFLARE_API_TOKEN` from `cloudflared-secrets`; reusing `TUNNEL_TOKEN` returns Cloudflare `400` auth failures and leaves `connections` null.
