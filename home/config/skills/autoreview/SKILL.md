---
name: autoreview
description: "Run a structured code review (Codex default) as a closeout check on a local or PR branch before commit or ship."
---
...
- user asks for Codex review / autoreview
...
- Treat heartbeat lines like `review still running: ... elapsed=... pid=...` as healthy progress, not a hang. Let the helper continue while heartbeats are advancing. Pass `--stream-engine-output` when live engine text is useful.
...
- If `gh` reports `database disk image is malformed`, retry once after refreshing GitHub CLI state; do not bypass `gh` unless freshness requires live GitHub.
...
- supports `--engine codex`; default is `AUTOREVIEW_ENGINE` or `codex`; Codex should remain the default when nothing is set
...
- supports `--stream-engine-output` or `AUTOREVIEW_STREAM_ENGINE_OUTPUT=1` for live engine text while preserving structured validation
