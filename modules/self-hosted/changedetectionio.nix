...
let
  changedetection-env = "/srv/apps/changedetectionio/changedetection.env";
  changedetection-profile-bootstrap = pkgs.writeTextFile {
    name = "changedetectionio-profile-bootstrap.py";
    executable = true;
    text = ''
...
in
{
  virtualisation.oci-containers.containers."changedetectionio" = {
    ...
    volumes = [
      "/srv/apps/changedetectionio:/datastore:rw"
    ];
  };

  systemd.services.podman-changedetectionio = {
  ...
  systemd.tmpfiles.rules = [
    "d /srv/apps/changedetectionio 0755 apps apps -"
  ];
}
