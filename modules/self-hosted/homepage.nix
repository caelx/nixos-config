      ...
      if [ -d "/srv/apps/homepage" ]; then
        ...
        if [ -f "$SERVICES_FILE" ]; then
          ...
          service_args=(
            ...
            "[Services].[Hatchet].icon=literal:sh-hatchet"
            "[Services].[Hatchet].description=literal:Workflow Orchestrator"
            "[Services].[Hatchet].server=literal:chill-penguin"
            "[Services].[Hatchet].container=literal:hatchet"
          ...
          ${pkgs.yq-go}/bin/yq -i '
            (.[] | select(has("Services")) | .Services) |= map(select((has("Hermes") or has("Honcho") or has("Firecrawl") or has("Firecrawl Playwright") or has("PriceBuddy") or has("PriceBuddy Scraper") or has("Changedetection") or has("n8n") or has("SearXNG") or has("Windmill") or has("Prefect")) | not))
