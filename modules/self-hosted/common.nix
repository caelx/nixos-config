...
let
  ...
  dockerhub-auth-script = pkgs.writeShellScriptBin "podman-dockerhub-auth" ''
    ...
    EOF
          ...
          cat > /run/containers/0/auth.json <<EOF
    {"auths":{"https://index.docker.io/v1/":{"auth":"$AUTH"},"docker.io":{"auth":"$AUTH"}}}
    EOF
        fi
  ...
  dockerhub-authenticated-containers = lib.attrNames (
    lib.filterAttrs (
      _: container: builtins.substring 0 10 container.image == "docker.io/"
