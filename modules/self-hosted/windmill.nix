...
let
  ...
  imageTag = "1.601.1-official-lg-page-14";
  officialBaseImage = "ghcr.io/windmill-labs/windmill:main";
  windmillPackage = pkgs.windmill.overrideAttrs (old: {
    env = old.env // {
      JEMALLOC_SYS_WITH_LG_PAGE = "14";
    };
  });
  windmillClosure = pkgs.closureInfo {
    rootPaths = [
      windmillPackage
      pkgs.cacert
      pkgs.curl
  ...
  windmillImageContext = pkgs.runCommand "ghostship-windmill-official-overlay-context" { } ''
    mkdir -p "$out"

    while IFS= read -r path; do
      cp -a --parents "$path" "$out"
    done < "${windmillClosure}/store-paths"

    cat > "$out/Dockerfile" <<'EOF'
FROM ${officialBaseImage}
COPY nix/store /nix/store
ENV PATH="${windmillPackage}/bin:${pkgs.curl}/bin:${pkgs.util-linux}/bin:$PATH"
ENV SSL_CERT_FILE="${pkgs.cacert}/etc/ssl/certs/ca-bundle.crt"
ENV NIX_SSL_CERT_FILE="${pkgs.cacert}/etc/ssl/certs/ca-bundle.crt"
ENTRYPOINT ["${windmillPackage}/bin/windmill"]
CMD []
EOF
  '';
  buildImage = pkgs.writeShellScript "windmill-build-official-overlay-image" ''
    set -eu

    mkdir -p /run/ghostship-image-build
    exec 9>/run/ghostship-image-build/windmill.lock
    ${pkgs.util-linux}/bin/flock 9

    ${config.virtualisation.podman.package}/bin/podman build \
      --pull=always \
      --tag ${imageName}:${imageTag} \
      ${windmillImageContext}
  '';
