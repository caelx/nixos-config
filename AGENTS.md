...
- Hermes now treats Codex on `openai-codex/gpt-5.4` as the normal primary model lane, with fallback `opencode-go/minimax-m2.7`, router custom provider alias `coding`, and default `agent.reasoning_effort = "medium"`.
