...
- `hermes-secrets` should carry the shared Hermes runtime provider env that Ghostship actively uses today, but keep the dedicated internal `N8N_API_KEY` in `n8n-secrets`; treat GitHub or Home Assistant env as optional future wiring unless a change explicitly adds them.
- Hermes utility env projection on `chill-penguin` should read only the required utility-facing values from service-local secret bundles or generated runtime env files, write the selected auth subset to `/srv/apps/hermes/runtime.env`, and supply upstream with the container-wide source vars it expects, including per-profile `BROWSER_*_CDP_URL` values derived from the matching managed CloakBrowser profile ids. Upstream owns rendering the managed `assistant`, `operations`, and `supervisor` profile `.env` files from those inputs. Keep Synology on `http://192.168.200.106:5000/`, keep qBittorrent plus NZBGet URL-only while their control auth remains disabled in this stack, and keep the n8n contract narrowed to `N8N_URL` plus `N8N_API_KEY`.
...
- Hermes profile persona sources live in tracked repo files under
  ...
  target file is missing; once seeded, treat the host copy as operator-owned
...
- Gluetun on `chill-penguin` should stay on Gluetun's custom-provider WireGuard path for PIA. `podman-gluetun` should start from the cached winner at `/srv/apps/gluetun/pia-wireguard-selection.json` or do only a provisional latency pick when no cache exists, regenerate `/run/secrets/gluetun-runtime.env` from that active winner during startup, and rely on `gluetun-pia-selector` to rerank PF-capable WireGuard regions 5 minutes after boot and every 8 hours thereafter. The selector should keep the best endpoint per region, benchmark the top 5 regions with the active Eweka `NZBGET_SERVER1_*` NNTP path, and restart Gluetun only when a materially faster winner is found while keeping PIA VPN-side port forwarding plus qBittorrent/VueTorrent port reconciliation wired through Gluetun's native hooks and the generic `/v1/portforward` monitor path.
