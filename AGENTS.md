...
- Hermes now treats Codex on `openai-codex/gpt-5.5` as the normal primary model lane, with fallback `opencode-go/minimax-m2.7`, router custom provider alias `agentic`, Firecrawl as the managed web backend, and default `agent.reasoning_effort = "medium"`.
