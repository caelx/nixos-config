...
- **RomM iframe auth likely needs SameSite=None; Secure**: The running container's backend currently configures `RedisSessionMiddleware` with `same_site="strict"` when OIDC is disabled and `https_only=False` in `/backend/main.py`. In a cross-site iframe from `apps.ghostship.io` to `romm.ghostship.io`, that prevents the browser from sending the RomM session cookie even though the standalone site works. Treat session and CSRF cookie attributes as the primary header-level suspect.
